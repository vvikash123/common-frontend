...
import {
    fetchLoadMoreWebStoriesData,
  } from "@/pages/api";
const VideoListing = (props) => {

    const { langConstant = {}, translations, addLangPath, lang, marginTop, marginBottom, componentData, data, imageWidth ,total_records } = props;
    ...
    const loadMoreData = async () => {
        ...
        setPageNo(newPage);
        const url = `${process.env.API_URL}/api/getlisting?seopath=videos&pageno=${newPage}&itemcount=${limit}&origin=descktop&channel=1005`;
    ...
    return (data?.length > 0 &&
        <>
            <section className={style['VideoListing']} style={{ marginBottom: `${marginBottom}px` }} key={props.key}>
                <ContainerBox>
                    <CommonGridBox gridType={'gridBox'} changeStyle={'grid-1'} xGap={34} yGap={24} marginBottom={50}>
                        {videosData && videosData.slice(0, 15).map((item, index) => (
                            <div className={`${style['col']}`} key={index}>
                                ...
                                >
                                    <WidgetCard
                                        ...
                                        cardSettingData={
                                            [
                                                { ...commonProperties, type: 'title', widgetCardType: 'leadCard', widgetLayout: 'vertical', elementClass: 'videos-caption-listing', lineClamp: 2, marginBottom: 4, elementType: 'p', textIcon: 'blackRoundPlayIcon' },
                                                { ...commonProperties, type: 'small', widgetCardType: 'leadCard', widgetLayout: 'vertical', elementClass: 'videos-listing-small-tag', lineClamp: 'inherit', marginBottom: 0, elementType: 'p' },
                                            ]
                                        }
                                        verticalList={[
                                            {
