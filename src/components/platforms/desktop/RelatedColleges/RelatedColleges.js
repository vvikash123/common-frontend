import React from "react";
import s from "./RelatedColleges.module.scss";

import CommonGridBox from "../../amp/Widgets/CommonGridBox/CommonGridBox";
import CollagesWidget from "@/components/common/CollagesWidget/CollagesWidget";
import TitleComponent from "@/components/common/TitleComponent/TitleComponent";
import SlickSlideWrapper from "@/components/common/SlickSlideWrapper/SlickSlideWrapper";

const RelatedColleges = ({ data, isMobile,isUtmParams = false }) => {
  const origin = isMobile ? "mobile" : "desktop"
  return (
    <>
      <div className={s['RelatedColleges2']}>
        <CommonGridBox gridType={'gridBox'} marginBottom={20} changeStyle={'grid-1'} yGap={20} xGap={16}>
        <div className={s['RelatedCollegesTop']}>
          <div className={s['Left']}>
          <TitleComponent
            title={`Explore popular colleges from ${data?.data[0]?.location?.state}`}
            // link="/colleges"
            margin="0px 0 0px 0"
            headingLevel="h2"
          />
          {/* <p>Students who showed interest in SRM Institute of Science and Technology - B.Tech also showed interest in</p> */}
          </div>
          {/* <span className={s['TotalColleges']}>(Total 2200 Colleges)</span> */}
          </div>
        </CommonGridBox>
        {/* <CommonGridBox gridType={'gridBox'} changeStyle={isMobile ? 'grid-3' : 'grid-3'} yGap={20} xGap={16}> */}

        <SlickSlideWrapper
          arrows={true}
          dots={isMobile ? false : true}
          slidesToShow={isMobile ? 3 : 1.2}
          infinite={false}
          changeBodyStyle={"CollagesWidget"}
          marginBottom={0}
          slidesToScroll = {2}
          autoplay={true} 
          autoplaySpeed={3000}
        >
          {data?.data.map((item) => {
            return (
              <CollagesWidget
                className="grid-3"
                data={item}
                key={item?.collegeId}
                origin={origin}
                isMobile = {false}
                isUtmParams = {isUtmParams}
              />
            );
          })}

</SlickSlideWrapper>
        {/* </CommonGridBox> */}

      </div>
      <style jsx>
        {`
          .RelatedColleges{
  width: 100%;
     padding: 15px;
  border-top: 10px #224099 solid;
}

.TotalColleges{
  color: #224099;
  font-size: 12px;
  font-weight: bold;
  text-align: right;
  margin-top: 10px;
}
        `}
      </style>
    </>
  );
};

export default RelatedColleges;
