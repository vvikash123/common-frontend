...
function TrendingSearch(props) {
    const { marginBottom=0, componentData } = props;  
        
    return ( componentData?.data?.children?.length >  0 && 
        <section className={`${style['TrendingSearch']}`} style={{ marginBottom: `${marginBottom}px` }} key={props.key}>
            <ContainerBox>
                <Typography
                    elementType={componentData?.headingType ||  'p'}
                    textValue={componentData?.seeMore?.text ||  'Trending Searches'}
                ...
                <CommonGridBox xGap={8} yGap={8} changeStyle={'trending-search'} flexWrap={'wrap'}>
                    {componentData?.data?.children?.length > 0 && componentData?.data?.children?.map((data, idx) => (
