...
const VideoHub = (props) => {
  ...
  const { componentData, getMSID, sectionIndex, marginBottom=60,imageWidth=300, data } = props;
  ...
  return ( data?.length > 0 &&
    <>
      <section className={style['VideoGuidance']} style={{ marginBottom: `${marginBottom}px` }} key={props.key}>
        <ContainerBox>
          <TitleComponent
            titleType={'h2'}
          ...
          <CustomSlideBox marginBottom={20}>
            {componentData?.data?.children[0]?.keywords && componentData?.data?.children[0]?.keywords?.map((item, idx) => (
              <TrendingTags
                key={idx}
                ...
                /**
                 * Tab props:
                 * To populate the data onclick of headers on the same widget.. without redirecting.
                 */
                tab={
                  {sectionIndex: sectionIndex,widgetType: componentData?.widgetType, isTabView:true }
                }
              />
            ))}
          </CustomSlideBox>
          <div className={`${style['grid-3']}`}>
            {
              componentData?.data?.children && componentData?.data?.children.slice(0, 5).map((item, index) => (
