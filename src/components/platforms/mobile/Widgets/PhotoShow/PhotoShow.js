...
import QuickStories from "@/components/platforms/mobile/Widgets/QuickStories/QuickStories";
...
const PhotoShow = (props) => {
  ...
  const updateUrlAndTitle = useCallback((id, jumpid) => {
    const currentItemId = parseInt(id.split('_').pop());
    const currentdata = fullSlideList?.filter((value) => value.msid === currentItemId);
    ...
    if (currentdata?.length > 0) {
      ...
      const newUrl = `/${correctSeoPathArr?.join('/')}-photostory-${parentid}/${jumpid}`;
      ...
      sendGAPageView(newUrl, title, getGoogleTagCustomDimensions(info, currentItemId));
    }
  }, [fullSlideList]);
